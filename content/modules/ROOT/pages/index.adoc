...
*Virtual Machine Administrators* -- Those responsible for day to day management of virtual machines in OpenShift Virtualization. These users will often find themselves responsible for provisioning virtual machines, and day to day management of the virtual machines, and the applications running within.
...
* Remote access console uses the US keyboard layout by default, so it is good to know where special characters reside for other country's layouts, or to use the copy/paste function in a supported browser.
...
(Accurate as of November 7, 2025)
